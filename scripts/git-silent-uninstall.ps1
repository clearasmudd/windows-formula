# based on: https://github.com/Limech/git-powershell-silent-install/blob/master/git-silent-uninstall.ps1
...
{
  $confirmation = Get-Package -Provider Programs -IncludeWindowsInstaller -Name "Git*" -ErrorAction:SilentlyContinue
  if ($confirmation)
  {
    $host.SetShouldExit($LastExitCode)
    write-host $confirmation
    throw "Git not uninstalled"
  }
  else
  {
    write-host "All Git instances successfully uninstalled"
  }
}